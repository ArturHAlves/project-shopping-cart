...
export function totalPrice(price) {
  soma += price;
  span.textContent = soma.toFixed(2);
}
...
export async function addProductCart(ID) { // Vai add produtos no carrinho.
  saveCartID(ID); // vai salvar os parâmetros do createProductElement
  const { id, title, price, pictures } = await fetchProduct(ID); // vai pegar informações da API {id, title..}
  const cardProduct = createCartProductElement({ id, title, price, pictures });
  totalPrice(price);
  // console.log(cardProduct);
  const ol = document.querySelector('.cart__products'); // buscar o elemento
  ol.appendChild(cardProduct);
}
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', () => addProductCart(id));
